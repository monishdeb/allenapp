...
class _AllenAppLifeCycleDisplayState extends State<AllenAppLifeCycleDisplay> {
  ...
  void checkToNavigateToHome() async {
    ...
    // Only navigate if the pin code request timeout has expired
    // This prevents pushing the login route when returning from background
    if (last_pin_request != null && last_pin_request <= now) {
      if (!kIsWeb && (Platform.isAndroid || Platform.isLinux)) {
        navigatorKey.currentState?.pushReplacementNamed('/', arguments: {'forceLogin': true});
      }
      else if (kIsWeb) {
        navigatorKey.currentState?.pushReplacementNamed('/', arguments: {'forceLogin': true});
      }
